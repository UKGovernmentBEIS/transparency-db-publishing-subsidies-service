...
public class BulkUploadAwardsService {
	 ...
	 */
	public ValidationResult validateFile(MultipartFile file, String role,String accessToken,String grantingAuthorityGroupId,String userName) {
		...
		try {
			...
			if (validationResult.getValidationErrorResult().size() == 0) {
		        ...
		        UserDetailsResponse userDetailsResponse =getUserRolesByGrpId(accessToken,grantingAuthorityGroupId);
		        ...
		        for (UserResponse userResponse : users) {
		      	  ...
		      	  try {
		  			...
		  			EmailUtils.sendSingleAwardEmail(userResponse.getMail(),userName,String.valueOf(bulkUploadAwards.size()));
	...
	private List<ValidationErrorResult> validateSubsidyPurpose(List<BulkUploadAwards> bulkUploadAwards) {
		...
		//bulkUploadAwards.stream().forEach(System.out::print);
		...
		List<BulkUploadAwards> subsidyPurposeOthersErrorRecordsList = bulkUploadAwards.stream().filter(
				award -> ((award.getSubsidyObjective()!= null && ("Other".equalsIgnoreCase(award.getSubsidyObjective()) && (award.getSubsidyObjectiveOther()==null || StringUtils.isEmpty(award.getSubsidyObjectiveOther()))))))
				.collect(Collectors.toList());
		validationSubsidyObjectiveErrorListResultList.addAll(subsidyPurposeOthersErrorRecordsList.stream()
		...
		validationSubsidyObjectiveErrorListResultList.addAll(subsidyPurposeOtherErrorRecordsList.stream()
				.map(award -> new ValidationErrorResult(String.valueOf(award.getRow()), "D",
	...
	private List<ValidationErrorResult> validateNationaIdAwards(List<BulkUploadAwards> bulkUploadAwards) {
		...
		List<BulkUploadAwards> nationsIdCompanyNumberFormatErrorRecordsList = bulkUploadAwards.stream()
				.filter(award -> award.getNationalIdType()!=null && award.getNationalIdType().equalsIgnoreCase("Company Registration Number") && (award.getNationalId()!=null && (!validateCompanyNumber(award.getNationalId())))).collect(Collectors.toList());
